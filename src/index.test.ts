...
test("test 2 - group type of any array by compute result.", () => {
  ...
  // exec
  let result2: Array<Array<{ code: number; val: number }>> = GroupBy(
    arr2,
    (a, b) => a.code == b.code,
    false
  );
...
test("test 3 - specify the group to be assigned", () => {
  ...
  // exec
  let result3: Array<Array<number>> = GroupTo(arr3, (a: number) => {
    if (a < 10) return 0;
    else if (a >= 10 && a <= 50) return 1;
...
test("test 4 - random array group by test (array is not ordered)", () => {
  ...
  let arr4: Array<number> = new Array<number>(20)
    .fill(0)
...
test("test 5 - speed test (array is not ordered)", () => {
  ...
  let arr4: Array<number> = new Array<number>(1000000)
    .fill(0)
    .map(n => Math.ceil(Math.random() * 100));
  // log
